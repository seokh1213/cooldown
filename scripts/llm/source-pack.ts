 ...
 *   3. 위키 팁        플레이 팁, 상대 팁, 스킬 슬롯별 운용 노트 (영문)
 *   4. 패치 소급      현재 툴팁이 비어 있는 자리를 과거 패치에서 메운 것과, 지금은 사라진 설명
 *   5. 작성 지침      지금 스키마에서 무엇을 쓰고 무엇을 쓰지 말아야 하는지
 *
 * 사용:
 *   npm run llm:source-pack -- --champ Nasus
 *   npm run llm:source-pack -- --champ 나서스 --lane top
 *   npm run llm:source-pack -- --todo            # 아직 지식 카드가 없는 챔피언 목록
 */
...
function main() {
  ...
  if (argv.includes("--todo")) {
    ...
    const rows = data.champions
      .filter((champ) => !done.has(champ.id))
      .map((champ) => ({ id: champ.id, name: champ.name }))
      .sort((a, b) => a.id.localeCompare(b.id));
    console.log(`지식 카드 미작성 ${rows.length}종\n`);
    for (const r of rows) {
