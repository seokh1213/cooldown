...
export async function run(file: string, limit = Infinity) {
  ...
  const meta = (await (await fetch(`/models/judge/kev-b3.json`)).json()) as JudgeHeadMeta;
  const head = readJudgeHead(meta, await (await fetch(`/models/judge/kev-b3.bin`)).arrayBuffer());
