...
export function VsMatrixSkill(props: {
  ...
  version: string; groupStart: boolean; onSelect: (ability: AbilityV2, trigger: HTMLButtonElement) => void;
}) {
  const { t } = useTranslation();
  const { side, slot, name, ability, version, onSelect } = props;
  return (
    <th id={"vs-" + side + "-" + slot} data-testid={"vs-" + side + "-" + slot} scope="col" className={"border-b border-border/60 p-0 align-top font-normal " + (props.groupStart ? "border-l border-l-border/60" : "")}>
      <Tooltip>
