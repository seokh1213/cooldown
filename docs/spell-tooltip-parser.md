...
이 섹션의 예시는 `public/data/version.json`이 가리키는 현재 패치의
**크산테 W 스킬 데이터**를 기준으로 합니다. 패치 번호를 문서에 고정하지 않고,
데이터 생성기가 선택한 DDragon/CDragon 버전을 단일 기준으로 사용합니다.
...
안전하게 평가할 수 있는 선형 계산식만 정적 데이터로 내보냅니다.

- 원본 툴팁이 실제로 참조한 계산 키를 등장 순서대로 후보로 삼습니다.
- 후보를 감싼 `physicalDamage`, `magicDamage`, `trueDamage` 태그로 피해 유형을 정합니다.
- 기본값과 계수는 스킬 랭크 및 챔피언 레벨 축을 보존합니다.
- 수정 계산, 조건 계산, 다른 계산식 참조, 합과 안전한 곱은 재귀적으로 펼칩니다.
- 최대·현재·잃은 체력 비례 피해는 툴팁 문맥으로 종류가 확정될 때만 대상 체력에 적용합니다.
- 미니언·몬스터 전용 계산, 버프 중첩 수, 능력치끼리의 곱처럼 런타임 문맥이 필요한 식은
  임의로 근사하지 않고 `unsupported` 진단으로 남깁니다.

이 경계 덕분에 생성 단계에서 더 많은 원본 구조를 읽더라도 브라우저는 숫자 배열과
명시된 계수만 평가합니다. 새로운 CDragon 계산 파트는 먼저 회귀 테스트를 추가하고,
선형성 및 대상 조건을 증명한 뒤 지원 범위에 포함합니다.
